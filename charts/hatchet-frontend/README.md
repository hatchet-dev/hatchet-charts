...
A Helm chart for deploying the [Hatchet](https://hatchet.run) frontend static file server on Kubernetes.

> **⚠️ Important — this is an internal building block, not a chart you install directly.**

## Values validation

This chart ships a [`values.schema.json`](https://github.com/hatchet-dev/hatchet-charts/blob/main/charts/hatchet-frontend/values.schema.json). Helm validates your supplied values against it on `install`, `upgrade`, `template` and `lint`.
